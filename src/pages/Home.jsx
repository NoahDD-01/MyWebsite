...
import { Link } from 'react-router-dom';
import { motion } from 'framer-motion';
import { useTranslation } from 'react-i18next'; // 1. Hook import လုပ်ပါ

const Home = () => {
  const { t } = useTranslation(); // 2. t function ကို ယူသုံးပါ
    ...
    <div className="min-h-screen bg-[#0A0F2C] text-white overflow-hidden">
      ...
      <section className="flex flex-col items-center justify-center text-center px-6 py-24 md:py-40">
        ...
        >
          ...
          {t("HERO_TITLE").split("Future")[0]}
          <span className="text-[#FF5F5F]">{t("HERO_FUTURE")}</span>
          {t("HERO_TITLE").split("Future")[1]}
        </motion.h1>
